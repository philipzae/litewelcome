...
# Copyright © 2014-2015 "Linux Lite" <http://www.linuxliteos.com/contact.html>
#
...
class WelcomeConfig(object):
    def __init__(self):
        ...
        self._autostart_path = os.path.expanduser(
            '~/.config/autostart/lite_welcome.desktop')
    ...
    def change_autostart(self, state):
        if state and not os.path.exists(self._autostart_path):
            ...
            try:
                os.symlink('/usr/share/applications/lite_welcome.desktop',
...
class LiteAppView(WebKit.WebView):
    ...
    def _nav_request_policy_decision_cb(self, view, frame, net_req, nav_act,
        ...
        page = urlopen(uri)
        frame.load_string(page.read().decode("utf-8"), "text/html", "utf-8",
                          page.geturl())
        pol_dec.ignore()
        return True
    ...
    def _do_command(self, uri):
        ...
        if uri == 'start_updates':
            ...
            Popen(['exo-open', 'https://www.linuxliteos.com/hwdb.html'])
